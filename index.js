...
function itp2dds(buf) {
    ...
    while (ptr < buf.length) {
        ...
        switch (type) {
            case 'IHDR':
                ...
                tiled = data.readInt8(20) & 0x04
                dxgi_format = data.readInt8(22)
                ...
                switch (version) {
                    ...
                    case 3:
                        break;
                    default:
                        throw new Error('Unsupported ITP file version ' + version + ' try other tools.');
                }
                switch (dxgi_format) {
                    case 3: dxgi_format = 85; break; // BGR565
                    case 5: dxgi_format = 28; break; // RGBA8888
                    case 6: dxgi_format = 98; break; // BC7
                    default: throw new Error('Unsupported ITP file: unknown pixel format ' + dxgi_format);
                }
                break;
    ...
    return write_dds(images_raw[0], height, width, dxgi_format)
}

function write_dds(buf, height, width, dxgi_format) {
    let ddsHead = Buffer.from([
        0x44, 0x44, 0x53, 0x20, // magic: DDS 
        0x7C, 0x00, 0x00, 0x00, // head size
        0x07, 0x10, 0x00, 0x00, // flags: DDSD_CAPS | DDSD_HEIGHT | DDSD_WIDTH | DDSD_PIXELFORMAT
        ...
        0x20, 0x00, 0x00, 0x00, // DDS_PIXELFORMAT
        ...
        0x00, 0x10, 0x00, 0x00, // dwCaps: DDSCAPS_TEXTURE
        0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00,
        ...
        // DDS_HEADER_DXT10
        0x00, 0x00, 0x00, 0x00, // dxgiFormat
    ...
    ddsHead.writeUInt32LE(dxgi_format, 128)
